...
import { FaBook, FaCheck, FaSort, FaStar, FaTimes, FaUser } from 'react-icons/fa';
...
useEffect(() => {
    const fetchComments = async () => {
      try {
        ...
        const endpoint = filter === 'pending' 
          ? 'http://backend:9000/api/v2/pending' 
          : 'http://backend:9000/api/v2/approveComment';
            ...
            response.data.map(async (comment) => {
              ...
              try {
                const fullnameResponse = await axios.get(
                  `http://backend:9000/api/v2/GetNameApprenant/${comment.id}`
                );
              ...
              try {
                const formationResponse = await axios.get(
                  `http://backend:9000/api/v2/formation/${comment.id}`
                );
    ...
    const approveComment = async (commentId) => {
      try {
        await axios.put(`http://backend:9000/api/v2/${commentId}/approve`);
    ...
    const rejectComment = async (commentId) => {
      try {
        await axios.put(`http://backend:9000/api/v2/${commentId}/reject`);
        setComments(comments.map(comment => 
          comment.id === commentId 
