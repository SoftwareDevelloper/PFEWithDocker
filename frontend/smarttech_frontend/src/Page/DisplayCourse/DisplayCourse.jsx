...
const DisplayCourse = () => {
    ...
    useEffect(() => {
        if (course && course.category) {
          fetch(`http://localhost:9000/api/GetCourse/${course.category}`)
            .then((response) => response.json())
            .then((data) => {
                ...
                <>
                  ...
                  <div className="rights">
                    ...
                    <div className="inscription">
                        <Link to={`/Cours/${course.id}/Inscription`}>
                          <button className='inscrire'> {t('Inscription')} </button>
                        </Link>
                    </div>
        ...
        <Swiper modules={[Navigation, Pagination]} navigation pagination={{ clickable: true }} spaceBetween={20} slidesPerView={1}
                ...
                className="mySwiper">
         {
                ...
                <div className='lefts' key={relatedCourse.id}>
                    ...
                    <h1 className='coursetitle'>{currentLanguage === 'fr'
                        ? relatedCourse.titleFr
                        : currentLanguage === 'ar'
                        ? relatedCourse.titleAr
                        : relatedCourse.titleEn}
                    </h1>
                    <p className='coursedesc'>{currentLanguage === 'fr'
                    ? relatedCourse.descriptionFr
                    : currentLanguage === 'ar'
                    ? relatedCourse.descriptionAr
                    : relatedCourse.descriptionEn}</p>
                    <div className="prix">
                    <span className='courseRelatedPrice'>  {relatedCourse.price}  
                            {currentLanguage === 'fr'
                            ? "TND"
                            : currentLanguage === 'ar'
                            ? " دينار"
